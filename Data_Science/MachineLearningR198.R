...
housing = read.csv('../RTut/housing.csv')
...
#Run the above code first, because the bottom is what I worked on
...
# use sapply and ifelse to set value equal to one when the value in 
# ocean_proximity is equal to the column name
cat_housing[] <- sapply(seq_along(cat_housing), function(x)
  ifelse(names(cat_housing[x]) == as.character(housing$ocean_proximity), 1,0))
